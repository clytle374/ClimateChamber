...
void displayStatus(void) {
  ...
  switch (menuSelector) {
  case 0:
    targetLine(lineShift);
    u8x8.print(freeRam());
    u8x8.print(F(" FREERAM"));
    lineShift++;
    if (lineShift >= SCREEN_LINES_HIGH) {
      break;
    }
    [[fallthrough]];
  case 1:
  ...
  case 2:
    ...
    targetLine(lineShift);
    digitalClockDisplay(2);
    lineShift++;
    if (lineShift >= SCREEN_LINES_HIGH) {
      break;
    }
    targetLine(lineShift);
    digitalClockDisplay(3);
    lineShift++;
    if (lineShift >= SCREEN_LINES_HIGH) {
      break;
    }
  ...
  case 8:
    ...
    if (inHeatMode == true) {
      ...
      u8x8.setCursor(SCREEN_LINES_WIDTH - xShift - 1, lineShift);
      u8x8.print(pwmDrive, 0);
      u8x8.setCursor(15, lineShift);
      u8x8.print(F("%"));
    } else {
  ...
  case 9:
    ...
    targetLine(lineShift);
    if (digitalRead(LIGHT_PIN1) || digitalRead(LIGHT_PIN2) ||
        digitalRead(LIGHT_PIN3) || digitalRead(LIGHT_PIN4)) {
  ...
  case 10:
    ...
    targetLine(lineShift); // display airpump status
    u8x8.print(F("AIR SEC        "));
    u8x8.setCursor(15 - xShift, lineShift);
  ...
  case 11:
    ...
    targetLine(lineShift); // display huumidifier status
  ...
  case 15:
    ...
    targetLine(lineShift);
    displaySensorStatus(3);
    lineShift++;
    if (lineShift >= SCREEN_LINES_HIGH) {
      break;
    }
    [[fallthrough]];
    while (lineShift < SCREEN_LINES_HIGH){
      targetLine(lineShift);
      lineShift++;
    }
    break;
...
void targetLine(uint8_t row) {  //clear the line and set the cursor to the row
    u8x8.setCursor(0, row);
    for (int i = 0 ; i < SCREEN_LINES_WIDTH ; i++){
    u8x8.print(F(" ")); 
    }
    u8x8.setCursor(0, row); 
}
...
void displaySensorStatus(uint8_t idx) {
  if (sensor[idx].isValid == true) {
    u8x8.print(F("VALID   "));
  } else {
    u8x8.print(F("INVALID "));
  }
  if (sensor[idx].regenState == READY) {
    u8x8.print(F("READY  "));
  } else if (sensor[idx].regenState == FAILED) {
    u8x8.print(F("FAILED"));
  } else if (sensor[idx].regenState == REGEN) {
    u8x8.print(F("REGEN "));
  }
}
...
void displayUpdate(void) {
  unsigned long mins =
      millis() / 60000UL; //*********output uptime in minutes ***********
  switch (displayMode) {
  case RUN:
...
void displayProgram(void) {
  // program by default so we will used the lower part of the screen from
  // program mode.
  ...
  u8x8.setFont(u8x8_font_chroma48medium8_r);
  u8x8.setCursor(0, 0);
  // u8x8.print(parameterNames[currentParam]);  //really bad name? lol  Print
  // the name of the parameter
  char *currentStringPointer =
      (char *)pgm_read_word(&(parameterNames[currentParam]));
  ...
  u8x8.setCursor(0, 1);
  if (edit && (menuPointer ==
               10)) { // handle the pointer for parameter change selection
    u8x8.print(F("^^^^^^^^^^^"));
...
void digitalClockDisplay(uint8_t mode) { // digital clock display of the time
  ...
  case 2: {
    char *currentStringPointer =
        (char *)pgm_read_word(&(monthNames[month() - 1]));
    u8x8.print(currentStringPointer);
    u8x8.print(F(" "));
    u8x8.print(day());
    break;
